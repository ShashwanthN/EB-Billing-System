  ...
  useEffect(() => {
    const fetchPaymentStatus = async () => {
      try {
        ...
        const response = await axiosInstance.get("/payment/redirect/registration", {
          params: {
        ...
        const paymentStatus = response.status === 202 ? "paid" : "unknown";
        setMessage(
          paymentStatus === "paid" ? "Payment Successful!" : "Payment status unknown."
        );
    ...
    fetchPaymentStatus();
  }, [connectionId, connectionType, navigate]);
      ...
      <div className="relative z-10 text-center backdrop-blur-md border backdrop-brightness-75 p-6 md:p-10 rounded-lg border-trueGray-500 max-w-lg mx-4">
        ...
        {message && (
          <div>
            ...
            <p className="text-warmGray-300">Redirecting to home</p>
          </div>
        )}
        {error && (
          <div>
...
export default PaymentSuccessService;
